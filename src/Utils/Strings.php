...
class Strings
{
	...
	public static function toAscii($s)
	{
		...
		if (class_exists('Transliterator') && $transliterator = \Transliterator::create('Any-Latin; Latin-ASCII')) {
			...
			$s = preg_replace_callback('#[\x80-\x{10FFFF}]++#u', function($m) {
				return @iconv('UTF-8', 'WINDOWS-1250//TRANSLIT', $m[0]); // intentionally @
			}, $s);
			...
			$s = preg_replace_callback('#[\x80-\x{10FFFF}]++#u', function($m) {
				return @iconv('UTF-8', 'ASCII//TRANSLIT', $m[0]); // intentionally @
			}, $s);
		}
